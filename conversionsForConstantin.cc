...
"VHToNonbb.root", "GluGluHToZZTo4L_M125.root", "VBF_HToZZTo4L_M125.root", "WWG.root","WWW.root", "WWZ.root", "WWTo2L2Nu_DoubleScattering.root", "WWTo2L2Nu.root",  "ZZZ.root", "WZTo3LNu_mllmin01.root", "TTGJets.root","ZGTo2LG.root", "WGToLNuG.root", "TGJets.root", "TTJets_DiLept.root", "TTJets_SingleLeptFromTbar.root", "TTJets_SingleLeptFromT.root", "DYJetsToLL_M10to50.root", "DYJetsToLL_M50.root", "ttHToNonbb.root", "TTWJetsToLNu.root", "TTZToLLNuNu.root", "TTZToLL_M1to10.root",  "TTTT.root"};		
 ...
 0.9561, 0.01212, 0.001034,  0.2147, 0.2086, 0.1651, 0.1729,   12.178, 0.01398, 58.59*WZSF, 3.697, 123.9*XgammaSF, 489*XgammaSF,  2.967, 87.315, 182.175, 182.175, 18610, 1921.8*3, 0.215, 0.2043, 0.2529, 0.0493, 0.009103};
	...
	const TString extra = "_EWKinoConversionCR_OSSFele";	//for plot file names
	...
	const double histMax[nDist] = {150, 200, 150, 300, 300, 300, 300, 300, 300, 200, 200, 200, 300, 600, 10,10, 3.2, 3.2, 3.2, 40, 300, 400, 200, 200, 200, 2.5, 2.5, 2.5, 0.5, 0.5, 0.5,  0.1, 0.1, 0.1, 100, 100, 100, 2, 2, 2, 0.8, 0.8, 0.8, 0.1, 0.1, 0.1, 0.015, 0.015, 0.015, 0.15, 0.15, 0.15, 10, 10, 10, 200, 2.5};
	...
	for(unsigned sam = 0, effsam = 0; sam < nSamples; ++sam, ++effsam){
        ...
        for(Long64_t it = 0; it < nEntries; ++it){
			...
			//SKIP GLUGLU->ZZ ON REQUEST OF CONSTANTIN
			if(fileList[sam].Contains("GluGluToZZ") ) continue;
			//Baseline event selection:
			if(!baseline(true, true, false, true)) continue;
			...
			*/
			...
			if(fill == nSamples_eff && nBJets(false, true, 0) == 0){
				for(unsigned dist = 0; dist < nDist; ++dist){			
			...
			values[13] = HT;
			values[14] = static_cast<double>(nJets);
			values[15] = static_cast<double>(nBJets(false, true, 2));
			values[16] = fabs(METvec.DeltaPhi(lepV[0]));
			values[17] = fabs(METvec.DeltaPhi(lepV[1]));
			values[18] = fabs(METvec.DeltaPhi(lepV[2]));
			if(nBJets(false, true, 2) == 0){
				for(unsigned dist = 0; dist < nDist; ++dist){
			...
			values[13] = HT;
			values[14] = static_cast<double>(nJets);
			values[15] = static_cast<double>(nBJets(false, true, 0));
			...
			//vary btag SF down	
			for(unsigned dist = 0; dist < nDist; ++dist){
				histosDown[5][dist][fill]->Fill(TMath::Min(values[dist], maxBinC[dist]),  (scal/bTagSF(false, 0))*bTagSF(false, 1) );
			}
			//vary btag SF up
			for(unsigned dist = 0; dist < nDist; ++dist){
				histosUp[5][dist][fill]->Fill(TMath::Min(values[dist], maxBinC[dist]),  (scal/bTagSF(false, 0))*bTagSF(false, 2) );
			}
			//all pdf variations
			for(unsigned pdf = 0; pdf < 100; ++pdf){
				for(unsigned dist = 0; dist < nDist; ++dist){
				...
				*/
					if(bkg != nSamples_eff -1){
						...
						for(unsigned unc = 0; unc < nUnc; ++unc){
							...
							systbin += syst*syst;
							if(histos[dist][bkg + 1]->GetBinContent(b) == 0) systbin = 0;
							if(dist == 0 && histos[dist][bkg + 1]->GetBinContent(b) != 0){
								//std::cout << "syst " << uncNames[unc] << ": bin " << b << " = " << syst/histos[dist][bkg + 1]->GetBinContent(b) << std::endl;
								if(syst/histos[dist][bkg + 1]->GetBinContent(b) > 1) std::cout << "syst " << uncNames[unc] << ": bin " << b << " = " << syst/histos[dist][bkg + 1]->GetBinContent(b) << std::endl;
								//std::cout << "Syst = 
							}
						}
						...
						if(histos[dist][bkg + 1]->GetBinContent(b) == 0) syst = 0;
	...
	if(plotKinematics){
		for(unsigned dist = 0; dist < nDist; ++dist){
			plotDataVSMC(dataHistos[dist], bkgHistos[dist], eff_names, nSamples_eff, "conversionsConstantin/" +  histNames[dist] + extra, false, 0, "EWKino", bkgSystDist[dist]);
			plotDataVSMC(dataHistos[dist], bkgHistos[dist], eff_names, nSamples_eff, "conversionsConstantin/" +  histNames[dist] + extra + "_log", true, 0, "EWKino", bkgSystDist[dist]);
		}
	}
}
