...
void trilTree::Loop(){
	...
	for(unsigned sam = 0; sam < nSamples; ++sam){
		...
		if(sam > 0 && sam <= nSig){
			...
			cout << names[sam]  << " scale up unc = " << fabs(pdfCounter[sam -1][4] - hcounter[sam])/hcounter[sam] << endl;
			cout << names[sam]  << " scale up down = " << fabs(pdfCounter[sam -1][8] - hcounter[sam])/hcounter[sam] << endl;
		}
	...
	const TString uncNames[nUnc] = {"jec", "metUnclustered",  "scale", "pdf", "pu", "btagSF", "trigeff", "ideff", "scaleAcc"}; //"scaleAcc"};
	...
	for(unsigned sam = 0, effsam = 0; sam < nSamples; ++sam, ++effsam){
        ...
        for(Long64_t it = 0; it < nEntries; ++it){
			...
			if(conePt[0] > 30){
				yieldsDown[6][cat][fill]->Fill(controlR + 1, scal*0.98);
				yieldsUp[6][cat][fill]->Fill(controlR + 1, scal*1.02);
			} else{
				yieldsDown[6][cat][fill]->Fill(controlR + 1, scal*0.95);
				yieldsUp[6][cat][fill]->Fill(controlR + 1, scal*1.05);
			}
			//Id efficiency uncertainties
			unsigned flavorC = tril_flavorComb(ind, _flavors, lCount);
			if(flavorC == 0 || flavorC == 3){
				yieldsDown[7][cat][fill]->Fill(controlR + 1, scal*0.94);
				yieldsUp[7][cat][fill]->Fill(controlR + 1, scal*1.06);
			} else{
				yieldsDown[7][cat][fill]->Fill(controlR + 1, scal*0.9553);
				yieldsUp[7][cat][fill]->Fill(controlR + 1, scal*1.0447);	
			}
	...
	for(unsigned cat = 0; cat < nCat; ++cat){
		for(unsigned sig = 0; sig < nSig; ++sig){
			yieldsDown[8][cat][sig + 1] = (TH1D*) yields[cat][sig+1]->Clone();
			yieldsDown[8][cat][sig + 1]->Scale(pdfCounter[sig][8]/hCounter[sig+1]);
			yieldsUp[8][cat][sig + 1] = (TH1D*) yields[cat][sig+1]->Clone();
			yieldsUp[8][cat][sig + 1]->Scale(pdfCounter[sig][4]/hCounter[sig+1]);
	}
	...
	const unsigned nSyst = 11 + 1 + 2*nBkg; //9 general uncertainties + stat signal + stat bkg + extra unc per bkg
	TString systNames[nSyst] = {"lumi", "JEC", "pdfAcc", "metUncl", "scale", "pdf", "pu", "btagSF", "id_eff", "trigeff", "scaleAcc"};
	...
	for(unsigned proc = 0; proc < nBkg; ++proc){ //last background is non-prompt and isn't susceptible to these uncertainty sources (note that entry 0 is signal so that the loop goes up to the second to last bkg)
		...
		systUnc[9][proc] = 1.00;	//id eff
		systUnc[10][proc] = 1.00;	//trig eff	
		systUnc[11][proc] = 0;   //scale Acc
	}
	TString systDist[nSyst]; //probability distribution of nuisances
	for(unsigned syst = 0; syst < nSyst; ++syst){
		if(syst > 1 && syst < 12) systDist[syst] = "shape";
	...
	for(unsigned sig = 0; sig < nSig; ++sig){
		//Set correct pdf Acceptance uncertainty for this signal
		systUnc[1][0] = 1 + pdfUnc[sig];
		...
		for(unsigned cat = 0; cat < nCat; ++cat){
			...
			for(unsigned bin = 1;  bin < dataYields[cat]->GetNbinsX() + 1; ++bin){
				...
				//make root file 
				TFile* shapeFile = new TFile("./datacards/shapes/shapeFile_"  + sigN[sig] + "_bin" + (TString) std::to_string(binCounter) + extra + ".root", "recreate");
				...
				//print datacard
				hnl::printDataCard(dataYields[cat]->GetBinContent(bin), yields[cat][sig + 1]->GetBinContent(bin), "signal", bkgYieldVal, nBkg, bkgNames, systUnc, nSyst, systNames, systDist, "datacards/datacard" + sigN[sig] + "_bin" + (TString) std::to_string(binCounter) + extra, true, "shapes/shapeFile_" + sigN[sig] + "_bin" + (TString) std::to_string(binCounter) + extra);
	...
	//Print uncertainty ranges for each uncertainty source. 
	...
	for(unsigned cat = 0; cat < nCat; ++cat){
		...
		//Total bkg	histogram
		TH1D* bkgTot = (TH1D*) yields[cat][nSig + 1]->Clone();
		for(unsigned bkg = 1; bkg < nBkg; ++bkg){
			bkgTot->Add(yields[cat][nSig + 1 + bkg]);
		}
		//loop over uncertainties
		for(unsigned unc = 0; unc < nUnc; ++unc){
			std::cout << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
			std::cout << "Uncertainty source : " << uncNames[unc] << std::endl;
			TH1D* bkgUp = (TH1D*) yieldsUp[unc][cat][nSig + 1]->Clone();
			TH1D* bkgDown = (TH1D*) yieldsDown[unc][cat][nSig + 1]->Clone();
			for(unsigned bkg = 1; bkg < nBkg -1; ++bkg){
				bkgUp->Add(yieldsUp[unc][cat][nSig + 1 + bkg]);
				bkgDown->Add(yieldsDown[unc][cat][nSig + 1 + bkg]);
			}
			bkgUp->Add(yields[cat][nSamples_eff]);
			bkgDown->Add(yields[cat][nSamples_eff]);
			//loop over all bins and print the uncertainty range
			double maxUnc = 0.;
			double minUnc = 999999.;
			for(unsigned bin = 1; bin < bkgTot->GetNbinsX() + 1; ++bin){
				double unc = std::max(fabs(bkgUp->GetBinContent(bin) - bkgTot->GetBinContent(bin)), fabs(bkgDown->GetBinContent(bin) - bkgTot->GetBinContent(bin)) );
				if(unc/(bkgTot->GetBinContent(bin)) < minUnc){
					minUnc = unc/(bkgTot->GetBinContent(bin));
				}
				if(unc/(bkgTot->GetBinContent(bin)) > maxUnc){
					maxUnc = unc/(bkgTot->GetBinContent(bin));
				}
			}
			std::cout << "Maximum uncertainty = " << maxUnc*100 << "%" << std::endl;
			std::cout << "Minimum uncertainty = " << minUnc*100 << "%" << std::endl;
		}
	}
	
	//Make tables with all expected yields
	const TString backGroundNames[nSamples_eff] = {"ZZ/H", "triboson", "WZ", "X + #gamma", "TT/T + X",  "non-prompt"};
	for(unsigned cat = 0; cat < nCat; ++cat){
		hnl::makeTable(cat, bkgYields[cat], bkgSyst[cat], dataYields[cat], backGroundNames, nSamples_eff -nSig, catNames[cat]);
	}
	...
	for(unsigned cat = 0; cat < nCat; ++cat){
		...
		plotDataVSMC(dataYields[cat], bkgYields[cat], distNames, nSamples_eff - nSig, catNames[cat] + "_withSignal" + extra, true, 0, "HNL", bkgSyst[cat], true, signals,  sigPlotNames ,nSigToPlot);
		plotDataVSMC(dataYields[cat], bkgYields[cat], distNames, nSamples_eff - nSig, catNames[cat] + extra, true, 0, "HNL", bkgSyst[cat]);
	}
}
