...
void trilTree::Loop(){
	...
	for(unsigned cat = 0; cat < nCat; ++cat){
		for(unsigned effsam = 0; effsam < nSamples_eff + 1; ++effsam){
			...
			yields[cat][effsam]->Sumw2(); //kFALSE
	...
	*/
	...
	//print ZZ MT as fraction of total uncertainty
	//loop over all bins 
	...
	for(unsigned cat = 0; cat < nCat; ++cat){
		...
		for(unsigned b = 1; b < yields[cat][nSig + 1]->GetNbinsX() + 1; ++b){
			...
			std::cout << "search region = " << binCount << std::endl;
	...
	for(unsigned bkg = 0; bkg < nBkg; ++bkg){
		...
		for(unsigned cat = 0; cat < nCat; ++cat){
			...
			for(unsigned bin = 1; bin < bkgYields[cat][bkg]->GetNbinsX() + 1; ++bin){
				...
				//totalBkgYields[bkg]->SetBinErrorOption(TH1::kPoisson); // NEW NEW NEW 
