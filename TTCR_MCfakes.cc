...
"VHToNonbb.root", "GluGluHToZZTo4L_M125.root", "VBF_HToZZTo4L_M125.root", "WWG.root","WWW.root", "WWZ.root", "WWTo2L2Nu_DoubleScattering.root", "WWTo2L2Nu.root",  "ZZZ.root", "WZTo3LNu_mllmin01.root", "TTGJets.root","ZGTo2LG.root", "WGToLNuG.root", "TGJets.root", "TTJets_DiLept.root", "TTJets_SingleLeptFromTbar.root", "TTJets_SingleLeptFromT.root", "DYJetsToLL_M10to50.root", "DYJetsToLL_M50.root", "ttHToNonbb.root", "TTWJetsToLNu.root", "TTZToLLNuNu.root", "TTZToLL_M1to10.root",  "TTTT.root"};		
	...
	const double WZSF = 0.655;//0.654;
	const double ZZSF = 1.032;//1.075;
	const double XgammaSF = 0.945;//	0.939;
 ...
 0.9561, 0.01212, 0.001034,  0.2147, 0.2086, 0.1651, 0.1729,   12.178, 0.01398, 58.59*WZSF, 3.697, 123.9*XgammaSF, 489*XgammaSF,  2.967, 87.315, 182.175, 182.175, 18610, 1921.8*3, 0.215, 0.2043, 0.2529, 0.0493, 0.009103};
	...
	const TString extra = "_TT_MCfakes";	//for plot file names
	const bool plotKinematics = true;
	//////////////////////////
	TH1D* yields[nSamples_eff + 1];	//Total yields in every CR, to be used in simultaneous fit
	const TString eff_names[nSamples_eff] = {"data", "ZZ/H", "triboson", "WZ", "X + #gamma", "TT", "Drell-Yan", "TT/T + X"}; //X + #gamma
	const unsigned nDist = 57;  //Number of distributions to plot	
	TH1D* histos[nDist][nSamples_eff];	//Kinematic distributions to plot
	const unsigned nUnc = 6;//number of shape uncertainties
	const TString uncNames[nUnc] = {"jec", "metUnclustered", "pdf", "scale", "pu", "btagSF"};  //shape uncertainty names
	//Kinematic shape uncertainty histograms
	TH1D* histosDown[nUnc][nDist][nSamples_eff];
	TH1D* histosUp[nUnc][nDist][nSamples_eff];
	TH1D* histosPdfVar[100][nDist][nSamples_eff];
	...
	for(unsigned sam = 0, effsam = 0; sam < nSamples; ++sam, ++effsam){
        ...
        for(Long64_t it = 0; it < nEntries; ++it){
			...
			if(effsam > 0 && nBJets(true, false, 1) < 1)continue;
			else if(effsam == 0 && nBJets(true, false, 0) < 1) continue;
			...
			if(_flavors[mllI[0]] == _flavors[mllI[1]] && ( fabs(mll - 91.) < 15 || fabs(lepSyst.M() - 91.) < 15) ) continue;
			...
			*/
			...
			//nominal b-veto 
			if(nBJets(true, false, 0) < 1) continue;
			...
			//vary btag SF down	
			if(std::isnan(bTagSF(true, 0)) || std::isnan(bTagSF(true, 0)) || std::isnan(bTagSF(true, 0)) )  cout << "NAN" << endl;
			if(bTagSF(true, 0) != 0){
				for(unsigned dist = 0; dist < nDist; ++dist){
			...
			for(unsigned b = 1; b < bkgHistos[dist][bkg]->GetNbinsX() + 1; ++b){
				...
				*/
					if(bkg != nSamples_eff -1){
						...
						bkgSystDist[dist][bkg]->SetBinContent(b, histos[dist][bkg + 1]->GetBinContent(b)*0.3);
					}
	...
	if(plotKinematics){
		for(unsigned dist = 0; dist < nDist; ++dist){
			plotDataVSMC(dataHistos[dist], bkgHistos[dist], eff_names, nSamples_eff - 1, "controlR/" +  histNames[dist] + extra, false, 0, "HNL", bkgSystDist[dist]);
		}
	}
}
